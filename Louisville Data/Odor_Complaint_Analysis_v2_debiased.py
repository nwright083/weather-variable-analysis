...
def custom_show():
    global plot_counter
    plot_counter += 1
    filename = f"Louisville Data/odor_plot_{plot_counter}_debiased.png"
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f"Saved plot to {filename}")
    
    # Copy to artifact directory
    artifact_path = os.path.join("/Users/nawrig04/.gemini/antigravity-ide/brain/c18d086a-2ac0-4770-8b4a-94a15887ba69", f"louisville_odor_plot_{plot_counter}_debiased.png")
    try:
...
smell_file_path = 'smell_reports.csv'
if not os.path.exists(smell_file_path) and os.path.exists(os.path.join('Louisville Data', smell_file_path)):
    smell_file_path = os.path.join('Louisville Data', smell_file_path)
...
# Filter out sparse early years (2017-2018) for correlation analyses.
# These years have only 10-20 reports total and would dilute the signal.
# The full dataset is preserved in daily_df for ORI modeling.
daily_df['date_dt'] = pd.to_datetime(daily_df['date'])
daily_corr = daily_df[daily_df['date_dt'].dt.year >= 2019].copy()
print(f"Correlation analysis subset (2019+): {len(daily_corr)} days")
...
# Using 2019+ filtered data to exclude sparse early years
...
print("\n=== DAILY Pearson Correlations (2019+, Weekday/Weekend) ===")
print(corr_table)
...
print("\n=== MONTHLY Pearson Correlations (Seoul-Style, 2019+) ===")
...
print("\n=== DAILY Spearman Rank Correlations (2019+, robust to zero-inflation) ===")
...
fig.suptitle('Monthly Average Odor Complaints vs Meteorological Variables (Seoul-Style, 2019+)', fontsize=14, fontweight='bold', y=1.01)
plt.tight_layout()
plt.show()
...
# Filter to 2019+ for cleaner visualization
monthly_ori = monthly_ori[monthly_ori['date_dt'] >= '2019-01-01'].copy()
...
plt.title('Monthly Averaged ORI vs. Odor Complaints (2019+)', fontsize=14, fontweight='bold')
fig.tight_layout()
plt.show()
...
ax1.legend(loc='upper left', fontsize=10)
plt.title('Monthly ORI vs. Temperature Trend (2019+)', fontsize=14, fontweight='bold')
fig.tight_layout()
plt.show()
...
plt.savefig("Louisville Data/odor_plot_poisson_marginal.png", dpi=300, bbox_inches='tight')
plt.close()
plt.show = custom_show
...
if not os.path.exists(geojson_path) and os.path.exists(os.path.join('Louisville Data', geojson_path)):
    geojson_path = os.path.join('Louisville Data', geojson_path)
...
# Source coordinates
source_name = 'Rubbertown'
source_lat, source_lon = 38.221, -85.842
...
html_path = 'louisville_odor_map_debiased.html'
if not os.path.exists(html_path) and os.path.isdir('Louisville Data'):
    html_path = os.path.join('Louisville Data', html_path)
m.save(html_path)
...
ax.set_title('Publication Map: Daily Odor Complaints by ZIP Code (Louisville)', fontsize=14, fontweight='bold')
...
static_path = 'louisville_static_choropleth_debiased.png'
if not os.path.exists(static_path) and os.path.isdir('Louisville Data'):
    static_path = os.path.join('Louisville Data', static_path)
...
exp_cols_l = [f'exp_{name}' for name in emitters.keys()]
...
X_spatial = sm.add_constant(daily_zip[weather_vars_l + exp_cols_l])
