...
# 1. Load Data
import os
if os.path.exists("Pittsburgh Data"):
    weather_file = "Pittsburgh Data/open-meteo-complete_hourly.csv"
    smell_file = "Pittsburgh Data/smell_reports_pittsburgh.csv"
    output_file = "Pittsburgh Data/open-meteo-smell-merged.csv"
elif os.path.exists("open-meteo-complete_hourly.csv"):
    weather_file = "open-meteo-complete_hourly.csv"
    smell_file = "smell_reports_pittsburgh.csv"
...
# 2. Process DateTime
print("Converting weather data timestamps from UTC to America/New_York local time...")
weather_df['datetime_utc'] = pd.to_datetime(weather_df['time'], utc=True)
weather_df['datetime_local'] = weather_df['datetime_utc'].dt.tz_convert('America/New_York')
...
# Convert to Local Time (Pittsburgh, PA is in Eastern Time)
